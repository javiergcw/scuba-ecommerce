...
const ConsultarContratoPage = () => {
    ...
    const [signatureKey, setSignatureKey] = useState(0); // Key para resetear el canvas
            ...
            try {
                ...
                if (contractResponse && contractResponse.success && contractResponse.data) {
                    ...
                    const fieldMapping: Record<string, { label: string; type: string; options?: string[]; section?: string }> = {
                        // Campos básicos de firma - NO se usan, se mapean desde general_info
                    ...
                    // Excluir campos básicos que se mapean desde general_info
                    const excludedBasicFields = ['email', 'signer_name', 'identity_type', 'identity_number', 'company'];
                    foundFields.forEach(fieldName => {
                        if (fieldName !== 'signature' && !excludedBasicFields.includes(fieldName)) {
                    ...
                    const fields: (ContractField & { section?: string })[] = Array.from(allFieldNames)
                        .map(fieldName => {
                            ...
                            // Los campos básicos (email, signer_name, identity_type, identity_number) 
                            // se mapean desde general_info, no se muestran como campos separados
    ...
    const handleSignContract = async () => {
        ...
        // Validar que existan los campos de general_info necesarios para mapear
        if (!formFields.general_info_first_name || !formFields.general_info_last_name) {
            setSignError('Por favor, completa el nombre y apellido en la información general');
            return;
        }

        if (!formFields.general_info_email) {
            setSignError('Por favor, completa el email en la información general');
            return;
        }

        if (!formFields.general_info_document_type || !formFields.general_info_document_number) {
            setSignError('Por favor, completa el tipo y número de documento en la información general');
            return;
        }

        // Validar email
        const email = formFields.general_info_email;
        ...
        try {
            ...
            // Mapear campos repetidos desde general_info_* a signer_name, email, identity_type, identity_number
            const fields: Record<string, string> = {
                ...formFields,
                signature: signature,
                // Mapear campos repetidos desde general_info
                signer_name: `${formFields.general_info_first_name || ''} ${formFields.general_info_last_name || ''}`.trim(),
                email: formFields.general_info_email || '',
                identity_type: formFields.general_info_document_type || '',
                identity_number: formFields.general_info_document_number || '',
            };

            // Remover campos básicos duplicados si existen (signer_name, email, identity_type, identity_number)
            // ya que ahora se mapean desde general_info
            delete fields.signer_name;
            delete fields.email;
            delete fields.identity_type;
            delete fields.identity_number;
            
            // Re-agregar los campos mapeados
            fields.signer_name = `${formFields.general_info_first_name || ''} ${formFields.general_info_last_name || ''}`.trim();
            fields.email = formFields.general_info_email || '';
            ...
            if (response && response.success) {
                ...
                // Limpiar formulario
                setSignature('');
                setSignatureKey(prev => prev + 1); // Forzar reset del canvas
                const initialFields: Record<string, string> = {};
                requiredFields.forEach((field: ContractField) => {
                    initialFields[field.name] = '';
                });
